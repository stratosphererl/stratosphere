...
export function HeaderTextButton(props: { text: string; link: string }) {
  return (
    <a href={props.link}>
      ...
      <button
        type="button"
        className="header-text-padding text-size-32 montserrat-semibold-italic ocean-blue-stroke-2"
      >
        {props.text}
      </button>{" "}
    </a>
...
export function HeaderLinkButton(props: { text: string; link: string }) {
  return (
    <a href={props.link}>
      {" "}
      <button type="button" className="header-block-button">
        {props.text}
      </button>{" "}
    </a>
  );
...
export function ProfileDropdown(props: {
  ...
  return props.titles.map((item, i) => (
    <div className="flex flex-nowrap">
      <div style={{ width: 23, height: 62 }}></div>
      {i < 3 ? (
        <a href={props.links[i]}>
          {" "}
          <button className="mini-header-block-button-regular">
            {props.titles[i]}
          </button>{" "}
        </a>
      ) : (
