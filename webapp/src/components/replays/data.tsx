...
export default function ReplayData(props: {
  data: JSON;
  version: number;
  classname: string;
}) {
  ...
  const uploadDate = new Date(props.data.gameMetadata.uploadDate)
    .toUTCString()
    .substring(5, 22);
  const playingDate = new Date(props.data.gameMetadata.time * 1000)
    .toUTCString()
    .substring(5, 22);
  ...
  // Replay players
  const [blueName, orangeName, bluePlayers, orangePlayers] = getPlayers(
    props.data.players
  );
  ...
  return (
    ...
    <div
      className={`glass-inner round flex flex-wrap justify-center replay-data-mobile ${props.classname}`}
    >
      <div className="glass-inner mx-2 flex mt-2 title-box">
        <b>
          <i>{replayTitle}</i>
        </b>
      </div>
      <div className="score-column flex justify-center">
        {blueScore > orangeScore ? (
          <div className="sky-blue-stroke-2">
            <b>
              {blueScore} - {orangeScore}
            </b>
          </div>
        ) : (
          <div className="orange-stroke-2">
            <b>
      ...
      <InfoColumn
        subtitles={["Uploaded", "Played", "Uploader", "Arena"]}
        info={[uploadDate, playingDate, uploaderUsername, mapName]}
        titleInfo="small-title"
      >
        <div>
          <b>{replayID}</b>
        </div>
      ...
      <InfoColumn
        subtitles={["Duration", "Season", "Gamemode", "Gametype"]}
        info={[duration, season, gamemode, gametype]}
        titleInfo=""
      >
        <div>
          <b>{avgRank}</b>
      ...
      <TeamColumn
        name={blueName}
        teamTextStyle="sky-blue-stroke-1"
        playerNames={bluePlayers}
      />
      <TeamColumn
        name={orangeName}
        teamTextStyle="orange-stroke-1"
        playerNames={orangePlayers}
      />
...
export function InfoColumn(props: {
  ...
  return (
    <div className="info-column flex flex-wrap justify-center">
      <div
        className={`${fullWidthCentered} title truncation ${props.titleInfo}`}
      >
        <b>{props.children}</b>
      </div>
      <div className={fullWidthCentered}>
        <b>
          <u>{props.subtitles[0]}</u>
        </b>
        : {props.info[0]}
      </div>
      <div className={fullWidthCentered}>
        <b>
          <u>{props.subtitles[1]}</u>
        </b>
        : {props.info[1]}
      </div>
      <div className={fullWidthCentered}>
        <b>
          <u>{props.subtitles[2]}</u>
        </b>
        : {props.info[2]}
      </div>
      <div className={fullWidthCentered}>
        <b>
          <u>{props.subtitles[3]}</u>
        </b>
        : {props.info[3]}
      </div>
    </div>
...
export function TeamColumn(props: {
  ...
  return (
    <div className="team-column flex flex-wrap justify-center">
      <div
        className={`${fullWidthCentered} ${props.teamTextStyle} title truncate`}
      >
        <b>{props.name}</b>
      </div>
  ...
  if (props.version === 0) {
    return (
      <div className="button-column flex flex-wrap">
        <a
          className="data-inverted-primary-btn top-btn flex justify-center"
          href={`/replay/${props.replayID}`}
        >
          Full Data
        </a>
        <a
          className="data-primary-btn mid-btn"
          onClick={() => console.log("2D View button clicked!")}
        >
          2D View
        </a>
        <a
          className="data-primary-btn bot-btn"
          onClick={() => toClipboardWithAlert(props.replayID)}
        >
          Share
        </a>
      </div>
