...
export default class ResponseDataWrapper {
  ...
  private getTeamTugEntry(isOrange: boolean): TeamTugEntry {
    const team: any = this.data.teams.find(
      (team: any) => team.isOrange == isOrange
    );
    const players: any[] = this.data.players.filter(
      (player: any) => player.isOrange == isOrange
    );
    ...
    const tugEntry = {
      ...
      saves: players
        .map((player: any) => player.saves)
        .reduce((a: number, b: number) => a + b, 0) as number,
      shots: players
        .map((player: any) => player.shots)
        .reduce((a: number, b: number) => a + b, 0) as number,
      assists: players
        .map((player: any) => player.assists)
        .reduce((a: number, b: number) => a + b, 0) as number,
      ...
      demos: players
        .map((player: any) => player.stats.demoStats?.numDemosInflicted ?? 0)
        .reduce((a: number, b: number) => a + b, 0) as number,
      boost: players
        .map((player: any) => player.stats.boost.boostUsage)
        .reduce((a: number, b: number) => a + b, 0) as number,
    };
  ...
  private getTeamsPlayers() {
    const team1 = this.data.players.filter(
      (player: any) => player.isOrange == 0
    ) as any[];
    const team2 = this.data.players.filter(
      (player: any) => player.isOrange == 1
    ) as any[];
    ...
    const addBoostData = (team: any[]) => {
      team.forEach((player: any) => {
        ...
        boostData.push({
          ...
          time_decent:
            this.data.gameMetadata.length -
            boostStats.timeFullBoost -
            boostStats.timeLowBoost -
            boostStats.timeNoBoost,
  ...
  getPlayerPositionViaBall() {
    ...
    players.forEach((player: any) => {
      positionData[0][player.name] =
        player.stats.positionalTendencies?.timeBehindBall ?? 0;
    });
    players.forEach((player: any) => {
      positionData[1][player.name] =
        player.stats.positionalTendencies?.timeInFrontBall ?? 0;
    });

    return [
      positionData as [any, any],
      players.map((player: any) => player.name) as string[],
    ];
  }
  ...
  private getFieldPositioning(positionalTendencies: any) {
    return {
      ...
      defendingThird: (positionalTendencies?.timeInDefendingThird ??
        0) as number,
      neutralThird: (positionalTendencies?.timeInNeutralThird ?? 0) as number,
      attackingThird: (positionalTendencies?.timeInAttackingThird ??
        0) as number,
    };
  }
  ...
  getBallPositionData() {
    ...
    const tendencies = this.getFieldPositioning(
      this.data.gameStats.ballStats.positionalTendencies
    );
    const positions: { x: number; y: number }[] = [{ x: 0, y: 0 }];
