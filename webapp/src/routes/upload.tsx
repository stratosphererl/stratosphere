...
export default function Upload() {
    ...
    const {progress, status} = useProgress({defaultStatus: "Ready for kickoff!"})
    ...
    useEffect(() => {
      ...
      const id = setInterval(() => {
        ...
        const chosenIncrement = incrementBy[Math.floor(Math.random() * incrementBy.length)];
    ...
    return (
        <MainPane className="max-w-[800px]" title="Upload">
            <iframe className="m-auto round border-white w-full" src={videoURL} height="315"  title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"></iframe>
            {
              !loggedIn ?
              <div className="mt-10">
                <p className="text-center">You must be logged in to upload replays.</p>
                <div className="flex justify-center">
                  <a className="mt-3 primary-btn" href="/login">Login</a>
                </div>
              </div>
            :
            <div>
              ...
              <div className="flex justify-center mt-10">
                {
                  ...
                  <button onClick={startProgress} disabled={!file} className="primary-btn">Upload</button>
                }
              </div>
            </div>
            }
