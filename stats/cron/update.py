...
def fake_endpoint(): # TODO: un-mock endpoint and get rid of this method
    mock_replays_file = open("mock_replays.json", "r")
    mock_replays_data = json.load(mock_replays_file)

    mock_users_file = open("mock_users.json", "r")
    mock_users_data = json.load(mock_users_file)
...
def update_stats_db(testing = False):
    ...
    replay_data, user_data = fake_endpoint() # Replace this with the un-mocked calls from the lines above (lines 22, 23)
